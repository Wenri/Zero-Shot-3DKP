...
# Original found here: https://github.com/nv-tlabs/MvDeCor/blob/main/src/render_utils.py
...
# from vis_utils import convert_o3d_to_trimesh, convert_trimesh_to_o3d
import time
...
class NormalShader:
    ...
    def get_program(
        ...
        self.program = pyrender.shader_program.ShaderProgram(
            vertex_shader="third_party/mesh_to_sdf/shader/mesh.vert",
            fragment_shader="third_party/mesh_to_sdf/shader/mesh.frag",
            defines=defines
        )
        return self.program
...
def pyrender_rendering(mesh, camera_poses, viz=False, light=False, intensity=6.0):
    ...
    images = []
    depth_images = []
    for _, i in enumerate(range(camera_poses.shape[0])):
...
def create_uniform_camera_poses(distance=2, resoultion=7):
    mesh = geometry.TriangleMesh()
    frontvectors = np.array(mesh.create_sphere(radius=distance, resolution=resoultion).vertices)
    camera_poses = []
...
def create_random_uniform_camera_poses(distance=2, low_scale=0.51):
    ...
    frontvectors = rotate_rotation_point_cloud(
        np.expand_dims(frontvectors, 1), angle_range=0.25
    )
