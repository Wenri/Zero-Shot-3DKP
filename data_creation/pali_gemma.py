...
from big_vision.evaluators.proj.paligemma.transfers import segmentation as segeval
...
class PaliGemma:
    def __init__(self, model_id="google/paligemma2-3b-mix-224"):
        # load the processor
        self.model = PaliGemmaForConditionalGeneration.from_pretrained(model_id, device_map='auto')
    ...
    def image_text_token(self, image, text='point to the salient keypoints in this image'):
        ...
        inputs = self.processor(text, image, return_tensors="pt").to("cuda")
    ...
    def draw_points(image, kps, radius=None, width=None, colors=None):
        detect, segment = kps
        boxes_mask = Image.new('RGB', (image.width, image.height), 0)
        try:
            boxes, _ = parse_bbox_and_labels(detect)
            draw_boxes(boxes_mask, boxes, (image.width, image.height))
            boxes_mask = Image.fromarray(np.asarray(boxes_mask).any(axis=-1))
        except Exception as e:
            print(f"Error parsing bounding boxes: {e}")
            boxes_mask = None

        try:
            boxes, masks = parse_segments(segment)
            new_image = draw_segment_output(image, boxes, masks, (image.width, image.height))
            image.paste(Image.fromarray(new_image), mask=boxes_mask)
        except Exception as e:
            print(f"Error parsing segments: {e}")

        if radius:
            image.putalpha(Image.fromarray(np.asarray(image).any(axis=-1).astype(np.uint8) * 127))
...
def main():
    ...
    t1 = time.time()
    print(t1 - t0)
    gemma.draw_points(image, generated_text)
