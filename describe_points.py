...
class Generator(RenderO3D):
    ...
    # @torch.inference_mode()
    # def get_prompts_molmo(self, tensor_images):
    ...
    def main_loop(self,vis=True, batch_size=1, debug=debug_enabled()):
        ...
        for mesh, class_title, mesh_id, annotations in self.io.loop_over_test_datasets():
            with nullcontext() if True else suppress(Exception):
                ...
                # Get the point with the maximum saliency
                ...
                # Give the selected point lcoation to draw the red sphere
                ...
                # Getting name of part
                if self.molmo is None:
                    self.molmo = self.Multimodal()
                image = Image.fromarray(rearrange(images[0], 'c h w -> h w c').cpu().numpy())
                molmo_prompt = self.molmo.generated_kps_points(image, text=f"Precisely name the part of the {class_title} marked by the red dot. Answer with just a name.")
...
if __name__ == '__main__':
    Generator(Path.home().joinpath("Desktop", "Zero-Shot-3DKP", "DescribePointsResults")).main_loop()
